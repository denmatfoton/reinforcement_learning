...
load_checkpoint_path = 'res/ddpg_128x256/'
training = False
if len(sys.argv) > 1:
...
def save_hyper_params(f, agent):
    ...
    f.write(str(agent.actor_local))
    f.write("DNN architecture, critic network:\n")
    f.write(str(agent.critic_local))
...
def ddpg(res_path, n_episodes=2000, max_t=300, print_every=10):
    """Deep Q-Learning.
